...
#--- CREATE OBJECT ---------------------------------------------------------------------------------------------------------+
    def __init__(self, path):
        self.name     = 'drl_pale'                                          # Fill structure
    ...
    # De 9 à 10 si on fait varier la cambrure : jouent sur l'abscisse du point qui définit la cambrure 
        ...
        self.x_min       = np.array([0.005, 0, 0, 0, 0, -0.08, -0.1, -0.1,
         0.3, -0.1])
        self.x_max       = np.array([0.05, 0.07, 0.12, 0.12, 0.15, 0., 0., 0.,  # La cambrure peut bouger de -0.03 à +0.03
         0.8, 0.3]) 
       

        #### Param de mon profil
        self.x_0         =  np.array([ 0.02312831,  0.04979696,  0.09752639,  0.09891683,  0.06405409,
       -0.04646897, -0.05629325, -0.01144421,  0.72674268, -0.00904017])                                       #Le naca 63415
        self.area        = 0
        self.corde       = 0.091  
        self.angle       = -0.218
        self.area_target = 0.000784           
    ...
    def shape_generation_dussauge(self, control_parameters):
        ...
        try:
            ...
            point2 = model.geo.addPoint(14/4, 0, 0, lc)
            point3 = model.geo.addPoint(14/4, 8/4, 0, lc)
            point4 = model.geo.addPoint(0, 8/4, 0, lc)
            ...
            with open(self.effort+'/Efforts.txt', 'r') as f:
                ...
                for ligne in f :
                    ...
                    if cx*cy == 0.:
                        ...
                        L_finesse.append(cy)                    ############# On touche surtout à cy 
