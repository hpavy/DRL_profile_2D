...
#--- CREATE OBJECT ---------------------------------------------------------------------------------------------------------+
    def __init__(self, path):
        ...
        self.act_size = 11                                                  # La cambrure à le droit de bouger
    ...
    # De 9 à 10 si on fait varier la cambrure : jouent sur l'abscisse du point qui définit la cambrure 
        ...
        self.x_min       = np.array([0.005, 0, 0, 0, 0, -0.08,-0.08 -0.1, -0.1,
         0.3, -0.1])
        self.x_max       = np.array([0.05, 0.07, 0.12, 0.07, 0.15, 0.,0., 0., 0.,  # La cambrure peut bouger de -0.03 à +0.03
         0.7, 0.3]) 
        self.x_0         = np.array([ 0.02359928,  0.04828674,  0.07590444,  0.08340451,  0.05728329,
        -0.04362299, -0.03815534, -0.03815534, -0.02214095,  0.7004262 , -0.00406199])
    ...
    def shape_generation_dussauge(self, control_parameters):
        ...
        ######## On mesh le nouvel airfoil avec immersion pour l'appartient 

        ### On mesh le nouvel airfoil
        mesh_size      = 0.0009                                               # Mesh size
        try:
    ...
    def compute_reward(self, control_parameters):
        ...
        begin_take_finesse = 200                                       # When we begin to take the reward 

        if finesse is not None :  
          # ANCIEB REWARD :  self.reward      = finesse[begin_take_finesse:].mean() 
            ## New reward
            reward_ema = ema(0.1,15)
            reward_ema.add_buffer(finesse[200:])
            self.reward      = reward_ema.avg()                        #finesse[begin_take_finesse:].mean() 
