...
def airfoil_plot(points, curve, title, control_parameter, file_save=None, xlim=None, ylim=None, ep=-1, finesse=None):
    ...
    if finesse is not None:
        plt.text(0.75, -0.4, f'Episode = {ep}\nL/D = {finesse}', bbox=dict(facecolor='white', alpha=0.8))
...
def plot_video(file, title, save_folder, title_video,ylim=(-0.05,0.15), cdt_init=None, title_init="", finesse_init=""):
    ...
    for i in range(df['Index'].max()):
        ...
        coeffs = df[["edge", "1","2","3","4","5","6","7","8","9"]][df['Reward']== df_max[i]].to_numpy()
        L_control.append(reconstruct_control_points(coeffs[0]))
        img.append(airfoil1(L_control[i],16))
        L_parameter.append(coeffs[0])
        airfoil_plot(L_control[i], img[i], title, L_parameter[i], file_save=save_folder+f'/video{i+1}',ylim=ylim, ep=i+1, finesse = "{:.2e}".format(df_max[i]))
